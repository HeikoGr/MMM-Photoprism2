...
function createAlbumIndex(options = {}) {
  let ttlMs = Number.isFinite(options.ttlMs) ? Math.max(0, options.ttlMs) : DEFAULT_TTL_MS;
  ...
  return {
    ...
    /**
     * Change the TTL, e.g. after the config changed. Takes effect on the next isStale().
     *
     * @param {number} value - TTL in ms; ignored unless finite
     */
    setTtl(value) {
      if (Number.isFinite(value)) {
        ttlMs = Math.max(0, value);
      }
    },
...
function buildImageUrl(photo, config, tokens = {}) {
  ...
  if (config?.useThumbnails) {
    // "auto" is resolved per display in the browser (lib/thumbnail-size.js),
    // which appends its own size to thumbnailBase; url is the fallback.
    const size = config.thumbnailSize && config.thumbnailSize !== "auto" ? config.thumbnailSize : "fit_1920";
    const token = tokens.preview || tokens.download || "public";
    const thumbnailBase = `${apiUrl}/api/v1/t/${file.Hash}/${token}`;
    return {
      url: `${thumbnailBase}/${size}`,
      thumbnailBase,
      fileHash: file.Hash,
    };
  }

...
const RUNTIME_PATHS = [/^node_helper\.js$/, /^MMM-Photoprism2\.js$/, /^MMM-Photoprism2\.css$/];

// Carve-outs inside those trees that are not runtime behavior.
const RUNTIME_EXCEPTIONS = [/\.md$/];
...
function main() {
  if (process.env.PHOTOPRISM2_ALLOW_SCOPE_MISMATCH === '1') return;
...
${shown.map((file) => `    - ${file}`).join('\n')}${more > 0 ? `\n    ... and ${more} more` : ''}
  ...
  Deliberate exception:
    PHOTOPRISM2_ALLOW_SCOPE_MISMATCH=1 git commit ...
`);
  process.exit(1);
}
